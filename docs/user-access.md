...
Auth invitation and the `staff` insert are separate operations. If the email is sent but the insert fails, the function returns an error for dashboard repair; the recipient cannot access private stock without a `staff` row. Supabase's default email service has rate limits, so a larger team may need custom SMTP. If the team later needs separate permissions for receiving, dispatching, reporting, or multiple Founders, revisit the three-role model and add explicit permission records and an access-change audit log.
