...
const roleLabel = { founder: "ผู้ก่อตั้ง", staff: "ผู้แก้ไข" };
...
let staffNames = {};
let staffList = []; // แถวทั้งหมดของตาราง staff (สำหรับหน้าจัดการผู้ใช้ของ Founder)
let isFounder = false;
let statusFilter = "";
let channel = null, reloadTimer = null, loading = false;
// เก็บ type จาก URL (#type=invite / #type=recovery) ไว้ก่อน supabase-js จะล้าง hash ทิ้ง
const authUrlType = (() => { try { return new URLSearchParams(location.hash.replace(/^#/, "")).get("type"); } catch (_) { return null; } })();
let invitePending = authUrlType === "invite" || authUrlType === "recovery" || authUrlType === "signup";
...
fetchAll(() => sb.from("staff").select("user_id,name,email,role,is_active").order("created_at"))
]);
items = new Map(its.map(r => [r.id, r]));
entries = mvs.map(m => ({ ...m, qty: Number(m.qty), date: m.date ? String(m.date).slice(0, 10) : null }));
staffList = st;
staffNames = {}; staffList.forEach(s => staffNames[s.user_id] = s.name);
const mine = staffList.find(s => s.user_id === session.user.id);
isFounder = !!(mine && mine.is_active && mine.role === "founder");
}
...
.on("postgres_changes", { event: "*", schema: "public", table: "staff" }, scheduleReload)
.subscribe();
}
...
session = s;
let staff = false;
if (s) { const { data } = await sb.rpc("is_staff"); staff = data === true; }
mode = staff ? "staff" : "visitor";
const isStaff = mode === "staff";
isFounder = false; staffList = [];
$("loginBtn").hidden = !!s; $("userMenu").hidden = !s;
$("meEmail").textContent = s ? s.user.email : "";
$("roleBadge").textContent = isStaff ? "ผู้แก้ไข" : "ผู้เยี่ยมชม";
$("roleBadge").classList.toggle("staff", isStaff);
$("roleBadge").classList.remove("founder");
$("tabs").hidden = !isStaff; $("actions").hidden = !isStaff; $("newItemBtn").hidden = !isStaff;
$("alerts").hidden = !isStaff; $("exportBtn").hidden = !isStaff; $("tabUsers").hidden = true;
...
const b = $("banner");
if (s && !isStaff) { b.hidden = false; b.textContent = "บัญชีนี้ยังไม่ได้รับสิทธิ์แก้ไขข้อมูล ติดต่อผู้ดูแลระบบให้เพิ่มสิทธิ์ ระหว่างนี้ดูได้เฉพาะยอดคงเหลือ"; }
else b.hidden = true;
...
$("tabUsers").setAttribute("aria-selected", String(which === "users"));
$("viewStock").hidden = which !== "stock"; $("viewLog").hidden = which !== "log"; $("viewUsers").hidden = which !== "users";
if (which === "log") renderLog();
if (which === "users") renderUsers();
}
$("tabStock").onclick = () => setTab("stock"); $("tabLog").onclick = () => setTab("log"); $("tabUsers").onclick = () => setTab("users");
...
$("tabUsers").hidden = !isFounder;
$("roleBadge").textContent = isFounder ? "ผู้ก่อตั้ง" : "ผู้แก้ไข";
$("roleBadge").classList.toggle("founder", isFounder);
$("roleBadge").classList.toggle("staff", !isFounder);
if (!isFounder && !$("viewUsers").hidden) setTab("stock");
if (!$("viewUsers").hidden) renderUsers();
}
...
if (!isFounder) return;
const arr = [...staffList].sort((a, b) => (a.role !== b.role ? (a.role === "founder" : -1 : 1) : (a.name || a.email).localeCompare(b.name || b.email)));
$("ucount").textContent = `${fmt(arr.length)} คน`;
$("userList").innerHTML = arr.length ? arr.map(s => `
<div class="urow" data-uid="${esc(s.user_id)}">
<div class="info"><b>${esc(s.name || s.email || "(ไม่มีชื่อ)")}${s.user_id === session.user.id ? " (คุณ)" : ""}</b><span>${esc(s.email || "–")}</span></div>
<span class="tag role-${esc(s.role)}">${esc(roleLabel[s.role] || s.role)}</span>
${s.is_active ? "" : `<span class="tag inactive">ปิดใช้งาน</span>`}
<button class="btn sm" type="button" data-edituser="${esc(s.user_id)}">แก้ไข</button>
</div>`).join("") : `<div class="state"><h2>ยังไม่มีผู้ใช้งาน</h2><p>กดเชิญผู้ใช้ใหม่ด้านบน</p></div>`;
}
$("userList").addEventListener("click", e => {
const b = e.target.closest("[data-edituser]"); if (!b) return;
const s = staffList.find(x => x.user_id === b.dataset.edituser); if (!s) return;
$("dEditUser").dataset.uid = s.user_id;
$("euTitle").textContent = s.name || s.email || "(ไม่มีชื่อ)"; $("euSub").textContent = s.email || "";
$("euRole").value = s.role; $("euActive").value = String(!!s.is_active); $("euName").value = s.name || "";
$("euMsg").textContent = "";
openDlg($("dEditUser"));
});
$("euSave").onclick = async () => {
const uid = $("dEditUser").dataset.uid;
const patch = { role: $("euRole").value, is_active: $("euActive").value === "true", name: $("euName").value.trim() };
$("euSave").disabled = true;
const { error } = await sb.from("staff").update(patch).eq("user_id", uid);
$("euSave").disabled = false;
if (error) { $("euMsg").textContent = /founder ที่ใช้งานอยู่/i.test(error.message || "") ? "ต้องมี Founder ที่ใช้งานอยู่อย่างน้อย 1 คนเสมอ ปรับผู้อื่นเป็น Founder ก่อนแล้วค่อยลดสิทธิ์รายนี้" : dbErr(error); return; }
$("dEditUser").close(); toast("บันทึกแล้ว"); reload();
};
$("inviteBtn").onclick = () => {
$("ivEmail").value = ""; $("ivName").value = ""; $("ivRole").value = "staff"; $("ivMsg").textContent = "";
openDlg($("dInvite")); setTimeout(() => $("ivEmail").focus(), 50);
};
$("inviteForm").addEventListener("submit", async e => {
e.preventDefault();
const email = $("ivEmail").value.trim(), name = $("ivName").value.trim(), role = $("ivRole").value;
$("ivSubmit").disabled = true; $("ivMsg").textContent = "";
try {
const res = await fetch(`${cfg.SUPABASE_URL}/functions/v1/invite-staff`, {
method: "POST",
...
if (!res.ok) {
const map = { already_registered: "อีเมลนี้มีบัญชีอยู่แล้วในระบบ", forbidden: "บัญชีนี้ไม่มีสิทธิ์เชิญผู้ใช้", invalid_email: "อีเมลไม่ถูกต้อง", unauthenticated: "เซสชันหมดอายุ ออกจากระบบแล้วเข้าใหม่", server_misconfigured: "เซิร์ฟเวอร์ยังไม่ได้ตั้งค่า Edge Function ครบ" };
$("ivMsg").textContent = map[body.error] || body.message || "เชิญผู้ใช้ไม่สำเร็จ";
return;
}
$("dInvite").close(); toast(`ส่งคำเชิญไปที่ ${email} แล้ว`); reload();
} catch (err) {
$("ivMsg").textContent = "เรียก Edge Function ไม่สำเร็จ ตรวจว่าติดีล่ง invite-staff แล้วและอินเทอร์เน็ตเชื่อมต่ออยู่";
} finally { $("ivSubmit").disabled = false; }
});
