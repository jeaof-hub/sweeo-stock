...
| คลังสินค้า | สิทธิ์ผู้ตรวจสอบ พร้อมส่ง แก้ไข และยกเลิกคำขอเบิกของตนเองที่ยังรออนุมัติ และดูสถานะ INV ของรายการตนเอง |
| แอดมิน | อนุมัติหรือปฏิเสธคำขอของคลังสินค้า จัดการ INV หลังส่งของ ส่งคำขอของตนให้เจ้าของ/ผู้ก่อตั้งอนุมัติ รับเข้า ลบรายการย้อนหลัง ปรับยอด เพิ่ม/แก้สินค้า ส่งออก Excel และดูรายการที่ถูกลบ |
...
| ไฟล์ | หน้าที่ |
|---|---|
| `index.html`, `style.css`, `app.js`, `scanner.js`, `i18n.js` | หน้าเว็บ กล้องสแกนสินค้า และคำแปลภาษาอังกฤษ |
...
| `supabase/16_temporary_delivery_notes.sql` | เพิ่มเลขใบส่งของชั่วคราว วันที่ส่งของ วัตถุประสงค์ และข้อมูลสำหรับเอกสาร PDF |
| `supabase/17_product_code_cleanup.sql` | แก้รหัสสินค้าที่เจ้าของตรวจยืนยันในฐานข้อมูล SWEEO ชุดปัจจุบัน |
| `supabase/18_post_delivery_invoices.sql` | บันทึก INV หลังส่งของ ผูกที่ระดับรายการ บังคับสิทธิ์ด้วย RPC และเก็บ change log |
...
8. รัน migration `08` ถึง `16` และ `18` ตามลำดับเลข โดย Production ที่ผ่าน migration ก่อนหน้าแล้วให้รันเฉพาะเลขที่ยังไม่ได้รัน ส่วน migration `17` เป็นการแก้ข้อมูลรหัสสินค้าจากไฟล์ที่เจ้าของตรวจแล้ว ใช้เฉพาะฐานข้อมูล SWEEO ชุดนี้
...
### การสแกนรหัสสินค้าด้วยกล้อง

ในฟอร์มเบิกและรับเข้า กด **สแกนรหัสสินค้า** เพื่อเปิดกล้องหลังบนโทรศัพท์ ระบบใช้ OCR ในเบราว์เซอร์และไม่ส่งภาพฉลากไปที่ Supabase หรือเซิร์ฟเวอร์อื่น ตัวอ่าน Tesseract และภาษาอังกฤษจะเริ่มดาวน์โหลดเมื่อกดสแกนครั้งแรกเท่านั้น จึงอาจใช้เวลาสักครู่ในครั้งแรก

ตัวสแกนเปรียบเทียบข้อความกับ `code` และ `model` ของสินค้า active ที่โหลดจากฐานข้อมูลโดยตรง จึงรองรับรหัสจริงทุกรูปแบบ รวมถึง `1196G30301T` โดยไม่จำกัดเฉพาะรหัสที่ขึ้นต้นด้วย `599` หากผลใกล้เคียงหลายรายการ ระบบจะแสดงตัวเลือกให้ผู้ใช้ยืนยัน และให้ Retail/Project อยู่ก่อนเมื่อคะแนนใกล้กัน ค่า `Q'TY ... PCS` บนฉลากจะแสดงเป็นจำนวนต่อลังเพื่อช่วยกรอกจำนวน แต่ไม่ถูกใช้เป็นรหัสสินค้า การสแกนไม่บันทึกหรือตัดยอดจนกว่าจะผ่านหน้าตรวจสอบรายการ

ตรวจตัวจับคู่กับไฟล์ JSON ที่ได้จาก `public_stock` ทั้งชุดได้ด้วย `node tools/validate_scanner_catalog.mjs public-stock.json` ตัวตรวจจะทดสอบรายการที่มีตัวระบุไม่ซ้ำ และยืนยันว่ารายการซ้ำไม่ถูกจับคู่ข้ามสินค้า

### การบันทึก INV หลังส่งของ

แอดมิน เจ้าของ และผู้ก่อตั้งใช้แท็บ **INV** เพื่อบันทึกเลขและวันที่จาก ERP แล้วเลือกรายการที่อนุมัติแล้วจากใบส่งของหลายใบมารวมใน INV เดียวกันได้ แต่ละรายการผูกได้เพียง INV เดียว รายการขายต้องผูก INV ส่วนสินค้าแถม เคลม และอื่น ๆ สามารถระบุว่าไม่ต้องเปิด INV พร้อมเหตุผลได้

ฟีเจอร์นี้ไม่ตัดหรือคืนสต็อก การสร้าง แก้ไข ถอดรายการ และยกเลิก INV ทำผ่าน RPC ที่ฐานข้อมูลและเก็บใน change log คลังสินค้าเห็นสถานะ INV เฉพาะคำขอของตนเองโดยแก้ไขไม่ได้ ช่องเอกสารเดิมของคำขอเก่าจะแสดงเป็น **เลขอ้างอิงเดิม**
