...
| สิทธิ์ | ทำอะไรได้ |
|---|---|
| ผู้เยี่ยมชม | ค้นหาสินค้าและดูยอดคงเหลือเท่านั้น ไม่เห็นประวัติ ลูกค้า ที่เก็บ หรือหมายเหตุ |
| ผู้ดูอย่างเดียว | ดูรายละเอียดสต็อก จุดสั่งผลิต ที่เก็บ และประวัติรับเข้า/ส่งออก รวมชื่อลูกค้าและเลขเอกสาร แต่แก้ไขข้อมูลไม่ได้ |
| ผู้แก้ไขสต็อก | บันทึกรับเข้า/ส่งออก ปรับยอดตามการนับ แก้ไข/เพิ่มสินค้า ดูประวัติ ส่งออก Excel |
| Foundator | สิทธิ์ผู้แก้ไขทั้งหมด พร้อมสร้างผู้ใช้ กำหนดรหัสผ่าน และเปลี่ยนสิทธิ์สมาชิกทีมในเว็บ |
...
8. ไปที่ **Edge Functions → Deploy a new function → Via Editor** ตั้งชื่อ `manage-users` แล้ววางโค้ดจาก `supabase/functions/manage-users/index.ts` และ Deploy จากนั้นปิด **Verify JWT with legacy secret** ใน Settings เพราะฟังก์ชันตรวจ JWT ผ่าน Supabase Auth เองและตรวจบทบาทใน `staff` ทุกครั้ง
9. Foundator เข้าสู่ระบบเว็บ กด **บัญชี → จัดการผู้ใช้** เพื่อสร้างสมาชิก เลือกสิทธิ์ และกำหนดรหัสผ่านเริ่มต้น ไม่มีอีเมลเชิญ Founder ต้องแจ้งรหัสผ่านให้เจ้าของบัญชีด้วยช่องทางที่เลือก และสมาชิกควรเปลี่ยนรหัสผ่านหลังเข้าสู่ระบบครั้งแรก
