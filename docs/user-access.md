...
Auth invitation and the `staff` insert are separate operations. If the invite request succeeds but the insert fails, the function returns an error for database repair; the recipient cannot access private stock without a `staff` row. Supabase's default email service is limited to organization member addresses, so configure custom SMTP before inviting web app users outside that group. An Auth `mail.send` event records an attempted send, not recipient delivery. If the team later needs separate permissions for receiving, dispatching, reporting, or multiple Founders, revisit the three-role model and add explicit permission records and an access-change audit log.
